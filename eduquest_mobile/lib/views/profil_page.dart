...
class _ProfilPageState extends State<ProfilPage> {
  ...
  // =====================================================================
  // 📍 TAMBAHAN BARU: Karena ProfilPage ditampilkan lewat IndexedStack di
  // DashboardPage, initState() TIDAK pernah dipanggil ulang saat pindah
  // tab (state-nya tetap hidup di memori). Akibatnya _loadUserData()
  // sebelumnya hanya berjalan SATU KALI seumur hidup halaman ini, jadi
  // poin/level/streak yang tampil di Profil tidak pernah ter-update
  // walau data di database sudah berubah (misalnya setelah menjawab
  // pertanyaan dan mendapat poin).
  //
  // didUpdateWidget() ini otomatis dipanggil Flutter setiap kali
  // DashboardPage mengirim `widget.user` yang baru (misalnya setelah
  // _refreshUserData() di Dashboard selesai jalan). Kalau datanya
  // memang berubah, kita panggil ulang _loadUserData() supaya statistik
  // di halaman ini (poin, level, progress bar, dsb) ikut ter-update —
  // TANPA perlu mengubah tampilan/UI apa pun.
  // =====================================================================
  @override
  void didUpdateWidget(covariant ProfilPage oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.user.points != widget.user.points ||
        oldWidget.user.petLevel != widget.user.petLevel ||
        oldWidget.user.streakCount != widget.user.streakCount ||
  ...
  Future<void> _loadUserData() async {
    ...
    var qCountRes = await db.rawQuery(
      'SELECT COUNT(*) as total, SUM(likes) as totalLikes FROM questions WHERE username = ?',
      [_currentUsername],
    );
    ...
    var qRes = await db.rawQuery(
      'SELECT SUM(comments) as total FROM questions WHERE username = ?',
      [_currentUsername],
    );
    _myQuestionsAnswered = (qRes.first['total'] as int?) ?? 0;

    // 3. Hitung: Jawaban yang SAYA berikan di forum & Total Like Jawaban
    var aRes = await db.rawQuery(
      'SELECT COUNT(*) as total, SUM(likes) as totalLikes FROM answers WHERE username = ?',
      [_currentUsername],
    );
    ...
    var aiRes = await db.rawQuery(
      'SELECT COUNT(*) as total FROM chats WHERE username = ?',
      [_currentUsername],
    );
    ...
    if (totalAktivitas > 0) {
      ...
      _forumUsagePercentage =
          (((myQuestionsCount + _myAnswersGiven) / totalAktivitas) * 100)
              .round();
    } else {
    ...
    // Memuat warna header
    int savedColor =
        prefs.getInt('header_color_$_currentUsername') ?? 0xFF6B48FF;
    _headerColor = Color(savedColor);

    // Memuat gambar header
    String? savedHeaderImage = prefs.getString(
      'header_image_$_currentUsername',
    );
    ...
    double savedHours =
        prefs.getDouble('app_usage_hours_$_currentUsername') ?? 0.0;
    _totalHoursUsage = savedHours == 0.0
        ? ((_streak * 1.5) + (totalAktivitas * 0.2))
        : savedHours;
    _totalHoursUsage += 0.1; // Bertambah sedikit setiap kali profil dibuka
    await prefs.setDouble(
      'app_usage_hours_$_currentUsername',
      _totalHoursUsage,
    );
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                leading: Icon(
                  Icons.image,
                  color: _level >= 5 ? Colors.blue : Colors.grey,
                ),
                title: Text(
                  "Pilih Gambar Latar",
                  style: TextStyle(
                    color: _level >= 5 ? Colors.black : Colors.grey,
                  ),
                ),
                trailing: _level < 5
                    ? Container(
                ...
                onTap: () async {
                  ...
                  if (_level < 5) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          "Bermain lebih banyak untuk capai Level 5 dan buka fitur ini!",
                        ),
  ...
  Widget _buildColorOption(Color color) {
    bool isSelected =
        _headerColor.value == color.value && _headerImageFile == null;
                  ...
                  try {
                    ...
                    if (context.mounted)
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Nama ini mungkin sudah dipakai orang lain.',
                          ),
                          backgroundColor: Colors.red,
                        ),
                      );
  ...
  Widget build(BuildContext context) {
    if (_isLoading)
      return const Scaffold(
        body: Center(
          child: CircularProgressIndicator(color: Color(0xFF6B48FF)),
        ),
      );
                    ...
                    child: Column(
                      children: [
                        _buildRowStatDetail(
                          ...
                          "Like Diterima (Pertanyaan)",
                          "$_totalQuestionLikes",
                        ),
                        const Divider(height: 24, thickness: 0.5),
                        _buildRowStatDetail(
                          Icons.thumb_up_alt,
                          "Like Diterima (Jawaban)",
                          "$_totalAnswerLikes",
                        ),
                        const Divider(height: 24, thickness: 0.5),
                        _buildRowStatDetail(
                          ...
                          "${_totalHoursUsage.toStringAsFixed(1)} Jam",
                        ),
                        const Divider(height: 24, thickness: 0.5),
                        _buildRowStatDetail(
                          Icons.question_answer_rounded,
                          "Pertanyaan Dijawab",
                          "$_myQuestionsAnswered",
                        ),
                        const Divider(height: 24, thickness: 0.5),
                        _buildRowStatDetail(
                        ...
                        _buildRowStatDetail(
                          ...
                          "Rata-rata Penggunaan Forum",
                          "$_forumUsagePercentage%",
                        ),
                        const Divider(height: 24, thickness: 0.5),
                        _buildRowStatDetail(
                          Icons.smart_toy_outlined,
                          "Rata-rata Penggunaan AI Helper",
                          "$_aiUsagePercentage%",
                        ),
                        const Divider(height: 24, thickness: 0.5),
                        _buildRowStatDetail(
                          Icons.fact_check_outlined,
                          "Rata-rata Akurasi Fakta",
                          "$_factCheckAccuracy%",
                        ),
                      ],
                    ),
                    ...
                    builder: (context, snapshot) {
                        ...
                        return Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              const SizedBox(height: 12),
                              Text(
                                "Kamu belum menjawab pertanyaan apapun.",
                      ...
                      return Column(
                        children: snapshot.data!
                            .map((data) => _buildAnswerHistoryCard(data))
                            .toList(),
                      );
                    },
                  ),
    ...
    return GestureDetector(
      onTap: () {
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) =>
                DetailPertanyaanPage(question: questionData, user: widget.user),
          ),
        );
      },
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                Icon(Icons.chat_bubble_outline, size: 16, color: _headerColor),
                const SizedBox(width: 8),
                const Text(
                  "Jawaban Saya:",
            ...
            Row(
              ...
              children: [
                Text(
                  data['answer_date'].toString().split(' ')[0],
...
// 📍 Bagian "Mode Gelap (Dark Mode)" sudah dihapus, sisanya tetap sama.
// ======================================================================
...
class _PengaturanPageState extends State<PengaturanPage> {
  Future<void> _bukaWebEduQuest(BuildContext context) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Membuka web EduQuest...'),
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text(
            'Apakah kamu yakin ingin menghapus foto profilmu saat ini?',
          ),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
